...
const { TypeNames, makeDefaultNonNullValue } = require('booben-types');
...
const arrayToObject = (
  ...
  array.reduce(
    (acc, cur, idx) =>
      includeFn(cur, idx, array)
        ? Object.assign(acc, {
            [keyFn(cur, idx, array)]: valueFn(cur, idx, array),
          })
        : acc,
    {}
  );
 ...
 */
const getter = key => object => object[key];
 ...
 * @type {Set<string>}
 */
const GQL_SCALAR_TYPES = new Set(['Int', 'Float', 'Boolean', 'String', 'ID']);

/**
 *
 * @type {Object<string, string>}
 */
const graphQLScalarTypeToBoobenType = {
  Int: TypeNames.INT,
 ...
 * @return {boolean}
 */
const isBuiltinGraphQLType = graphQLTypeName =>
...
const gqlTypeHasInterface = (type, interfaceName) =>
  ...
  !!type.interfaces.find(
    iface =>
      iface.kind === GQLTypeKinds.INTERFACE && iface.name === interfaceName
  );
...
const isRelayPageInfoType = type => {
  ...
  const hasNextPageField = findGQLField(
    type,
    RELAY_PAGEINFO_FIELD_HAS_NEXT_PAGE
  );
  ...
  const hasPreviousPageField = findGQLField(
    type,
    RELAY_PAGEINFO_FIELD_HAS_PREVIOUS_PAGE
  );
...
const pageInfoHasCursors = pageInfoType => {
  const startCursorField = findGQLField(
    pageInfoType,
    RELAY_PAGEINFO_FIELD_START_CURSOR
  );
  ...
  const endCursorField = findGQLField(
    pageInfoType,
    RELAY_PAGEINFO_FIELD_END_CURSOR
  );
...
const isRelayEdgeType = (type, schema) => {
  if (type.kind !== GQLTypeKinds.OBJECT) {
    return false;
  }

  if (!type.fields || type.fields.length < RELAY_EDGE_FIELDS_NUM) {
    return false;
  }
...
const isRelayConnectionField = (field, schema, relayTypes) => {
  ...
  const pageInfoField = findGQLField(
    connectionType,
...
const getAdditionalFieldsOnRelayConnection = (connectionField, schema) => {
  ...
  return connectionType.fields.filter(
    field =>
...
const parseGraphQLSchema = schema => {
  const queryTypeName =
    schema.queryType && schema.queryType.name ? schema.queryType.name : null;
  ...
  const haveRelay = !!relayTypes.nodeInterface && !!relayTypes.pageInfo;
  const retTypes = {};
  const retEnums = {};
  ...
  const includeGQLField = field => {
    ...
    return (
      typeKind !== GQLTypeKinds.INTERFACE && typeKind !== GQLTypeKinds.UNION
    );
  };
 ...
 */
const parseFieldName = fullFieldName => {
  const [fieldName, connectionFieldName = ''] = fullFieldName.split(
    CONNECTION_FIELD_NAME_SEPARATOR
  );
 ...
 * @return {string}
 */
const formatFieldName = (fieldName, connectionFieldName) => {
 ...
 * @return {string}
 */
const getTypeNameByField = (schema, fullFieldName, onType) => {
 ...
 * @return {string}
 */
const getTypeNameByPath = (schema, path, startType = '') =>
  path.reduce(
    (acc, cur) => getTypeNameByField(schema, cur, acc),
    startType || schema.queryTypeName
  );

const getBoobenTypeOfFieldMemo = new Map();
 ...
 * @param {DataSchema} schema
 * @param {string[]} source
 * @param {function(typedef: BoobenTypeDefinition): Object<string, Object>} sourceConfigsFromTypedef
 * @param {boolean} [checkRequired=false]
 * @return {BoobenTypeDefinition}
 */
const getBoobenTypeOfField = (
  fieldTypedef,
  schema,
  source,
  ...
  let memoBySchema = getBoobenTypeOfFieldMemo.get(schema);
  if (!memoBySchema) {
    memoBySchema = new Map();
    getBoobenTypeOfFieldMemo.set(schema, memoBySchema);
  }
  ...
  if (isBuiltinGraphQLType(fieldTypedef.type)) {
    ret = {
      type: graphQLScalarTypeToBoobenType[fieldTypedef.type],
    };
  ...
  if (deferredSubFields) {
    deferredSubFieldsTarget.fields = mapValues(deferredSubFields, subField =>
      getBoobenTypeOfField(
        subField,
        schema,
        source,
        sourceConfigsFromTypedef,
 ...
 */
const getBoobenValueDefOfField = (field, schema) =>
  getBoobenTypeOfField(field, schema, [], () => ({}));
 ...
 * @return {BoobenValueDefinition}
 */
const getBoobenValueDefOfMutationArgument = (arg, schema) =>
  getBoobenTypeOfField(
    arg,
    schema,
 ...
 * @return {BoobenValueDefinition}
 */
const getBoobenValueDefOfQueryArgument = (arg, schema) =>
  getBoobenTypeOfField(
    arg,
    schema,
 ...
 * @return {boolean}
 */
const fieldHasArguments = field => Object.keys(field.args).length > 0;
 ...
 * @return {DataField}
 */
const getFieldOnType = (schema, typeName, fullFieldName) => {
 ...
 * @return {?DataField}
 */
const getFieldByPath = (schema, path, rootTypeName = schema.queryTypeName) => {
 ...
 * @return {DataField[]}
 */
const getFieldsByPath = (schema, path, rootTypeName = schema.queryTypeName) => {
  const ret = [];
 ...
 * @return {number}
 */
const findFirstConnectionInPath = (
  schema,
  path,
